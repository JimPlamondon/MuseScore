...
void EnginePlayback::setInputParamPlain(const TrackSequenceId sequenceId, const TrackId trackId, const AudioResourceId& resourceId,
                                        uint32_t paramId, double plain)
{
    ONLY_AUDIO_ENGINE_THREAD;
    ITrackSequencePtr s = sequence(sequenceId);
    if (s) {
        s->audioIO()->setInputParamPlain(trackId, resourceId, paramId, plain);
    }
}
...
Ret EnginePlayback::doSaveSoundTrack(const TrackSequenceId sequenceId, const io::path_t& destination, const SoundTrackFormat& format)
{
#ifdef MUSE_MODULE_AUDIO_EXPORT
    ITrackSequencePtr s = sequence(sequenceId);
    if (!s) {

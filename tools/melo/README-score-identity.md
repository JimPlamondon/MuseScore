...
The macOS bundle exports `com.melopresto.score.document` and `com.melopresto.score.backup`, with the document icon and the Owner handler rank. Inherited MuseScore declarations are imported and their handlers are Alternate. The preview helper uses its own child bundle identifier and supports the native Score document type. Finder preview availability must be tested on the installed local package; building the helper alone does not prove the operating system will host it.
